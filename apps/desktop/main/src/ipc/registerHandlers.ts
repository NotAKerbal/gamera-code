...
export interface HandlerDeps {
  ...
  preview: {
    openPopout: (url: string, projectId?: string, projectName?: string) => Promise<{ ok: boolean }>;
    closePopout: () => Promise<{ ok: boolean }>;
    navigate: (url: string, projectId?: string, projectName?: string) => Promise<{ ok: boolean }>;
...
`
  ...
  const findProjectByPath = (path: string) => {
    ...
    return deps.repository.listProjects({ includeArchived: true }).find((project) => normalizePath(project.path) === normalized) ?? null;
  };
  ...
  ipcMain.handle(IPC_CHANNELS.projectsList, async (_event, input?: { includeArchived?: boolean }) => deps.repository.listProjects(input));
  ...
  ipcMain.handle(
    ...
    async (_event, input: { id: string; name?: string; path?: string; workspaceId?: string; color?: string }) => {
    return deps.repository.updateProject(input);
    }
  );

  ipcMain.handle(projectsArchiveChannel, async (_event, input: { id: string; archived: boolean }) => {
    return deps.repository.archiveProject(input.id, input.archived);
  });
  ...
  ipcMain.handle(
    projectTerminalWriteChannel,
    async (_event, input: { projectId: string; commandId: string; data: string }) => {
      return deps.projectTerminalManager.write(input.projectId, input.commandId, input.data);
    }
  );

  ipcMain.handle(
    projectTerminalResizeChannel,
    async (_event, input: { projectId: string; commandId: string; cols: number; rows: number }) => {
      return deps.projectTerminalManager.resize(input.projectId, input.commandId, input.cols, input.rows);
    }
  );

  ipcMain.handle(IPC_CHANNELS.previewOpenPopout, async (_event, input: { url: string; projectId?: string; projectName?: string }) => {
    return deps.preview.openPopout(input.url, input.projectId, input.projectName);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.previewNavigate, async (_event, input: { url: string; projectId?: string; projectName?: string }) => {
    return deps.preview.navigate(input.url, input.projectId, input.projectName);
  });

  ipcMain.handle(IPC_CHANNELS.previewOpenExternal, async (_event, input: { url: string }) => {
    return deps.preview.openExternal(input.url);
  });
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      input: { projectId: string; title: string; harnessId?: HarnessId; provider?: "codex" | "opencode" | "gemini" }
    ) => {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      input: {
        ...
        color?: string;
        harnessId?: HarnessId;
        provider?: "codex" | "opencode" | "gemini";
        status?: ThreadStatus;
        pinned?: boolean;
      }
    ) => {
  ...
  ipcMain.handle(
    ...
    async (_event, input: { harnessId?: HarnessId; provider?: Provider }) => {
  ...
  ipcMain.handle(
    ...
    async (_event, input?: { targets?: Array<"node" | "npm" | "git" | "rg" | "codex" | "opencode" | "gemini"> }) => {
  ...
  ipcMain.handle(
    installerGetAvailableModelsChannel,
    async (_event, input?: { opencodeBinaryOverride?: string; geminiBinaryOverride?: string }) =>
      deps.installerManager.getAvailableModels({
        opencodeBinaryOverride: input?.opencodeBinaryOverride,
        geminiBinaryOverride: input?.geminiBinaryOverride
      })
  );
