...
export interface HandlerDeps {
  ...
  codePanel: {
    openPopout: (projectName?: string) => Promise<{ ok: boolean }>;
    closePopout: () => Promise<{ ok: boolean }>;
  };
  webLink: {
...
`
  ...
  ipcMain.handle(codePanelOpenPopoutChannel, async (_event, input?: { projectName?: string }) => {
    return deps.codePanel.openPopout(input?.projectName);
  });
  ...
  ipcMain.handle(
    ...
    async (_event, input: { harnessId?: HarnessId; provider?: "codex" | "opencode" | "gemini" }) => {
